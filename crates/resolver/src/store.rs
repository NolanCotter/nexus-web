...
mod tests {
    use super::*;
    use crate::crypto::MockProvider;
    use crate::model::hex_encode;

    fn zone(name: &str) -> ZoneId {
        let mut z = [0u8; 32];
        let bytes = name.as_bytes();
        z[..bytes.len()].copy_from_slice(bytes);
        z
    }
    ...
    fn store() -> LocalStore<MockProvider> {
        let mut s = LocalStore::in_memory(MockProvider);
        s.bootstrap_root_key(zone("alice.nex"), [7u8; 32]);
        s
    }
}